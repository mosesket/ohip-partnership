...
export function print_country(setCountryOptions: any) {
  const countries = country_arr.map((country) => ({
    label: country,
    value: country,
  }));
...
export function print_state(state_index: any, setStateOptions: any) {
  const state_arr = s_a[state_index]?.split("|") || [];
  const states = state_arr.map((state: any) => ({
    label: state,
    value: state,
  }));
